...
def readMsg():
    msg = ""
    sys.stderr.write('IN readMsg()\n')
    while True:
        line = sys.stdin.readline()[0:-1]
        sys.stderr.write('MSGLOG: ' + line + '\n')
        if line == "end":
            break
        msg = msg + line + "\n"
    sys.stderr.write('MSGLOG2: ' + ''.join(msg) + '\n')
...
def readTaskIds():
    if pending_taskids:
        ...
        sys.stderr.write('About to readMsg bc no pending_taskids\n')
        msg = readMsg()
        while type(msg) is not list:
            pending_commands.append(msg)
            sys.stderr.write('About to readMsg bc type(msg) is not in list\n')
            msg = readMsg()
        return msg
